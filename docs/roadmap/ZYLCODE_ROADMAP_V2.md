...
| **Build unobserved** | `cargo build --workspace` cannot be executed in the build sandbox (build-script execution denied). One clean run outside the sandbox is required. **Not** a code defect. `cargo check --all-targets` and `clippy -D warnings` both **pass** on the full tree. |
